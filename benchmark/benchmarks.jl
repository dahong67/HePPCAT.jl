...
for d = 5:10:15, k = 1:3:4, L = 1:2
    ...
    for T = 0:10:20
        SUITE["sage"]["block: d=$d, k=$k, L=$L, T=$T"] = @benchmarkable HeteroscedasticPCA.ppca($Yblock, $k, $T, $F0)
        SUITE["sage"]["flat:  d=$d, k=$k, L=$L, T=$T"] = @benchmarkable HeteroscedasticPCA.ppca($Yflat, $k, $T, $F0)
    end
end
